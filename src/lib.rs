...
/// * `sniff_[name]` - reads only the nth record(s) from file.
...
macro_rules! read_py {
    (
        ...
        paste! {
            ...
            fn [< sniff_ $name _py >](infile: PathBuf, indices: Vec<i32>) -> PyResult<Vec<IndexMap<String, DmapField>>> {
                Ok([< $name:camel Record >]::sniff_file(&infile, &indices)
                    .map_err(PyErr::from)?
                    .into_iter()
                    .map(|rec| rec.inner())
                    .collect()
